...
local b, script = xpcall(function()
	return game:GetObjects("rbxassetid://114649230019871")[1]
end, function()
	return script
end)
...
do
	...
	Mouse,mouse,UserInputService,ContextActionService = game.Players.LocalPlayer:GetMouse(),game.Players.LocalPlayer:GetMouse(),game:GetService("UserInputService"),game:GetService("ContextActionService")
end
...
task.spawn(function()
	while true do
		...
		BMUSIC.SoundId = getcustomasset("ScriptBase/blind reaper.mp3")
		BMUSIC.Looped = true
		BMUSIC.Pitch = PITCH
...
task.spawn(function()
	...
	UIS.InputChanged:connect(function(io,gpe)
		if(io.UserInputType == Enum.UserInputType.MouseMovement)then
			UserEvent{Mouse=true,Target=mouse.Target,Hit=mouse.Hit}
		end
	end)
	...
	UIS.InputBegan:connect(function(io,gpe)
		...
		UserEvent{InputObject=true,KeyCode=io.KeyCode,UserInputType=io.UserInputType,UserInputState=io.UserInputState}
	end)
	...
	mouse.KeyDown:connect(function(k)
		UserEvent{KeyEvent='Down',Key=k}
	end)

	mouse.KeyUp:connect(function(k)
		UserEvent{KeyEvent='Up',Key=k}
	end)
...
if game.Players.LocalPlayer.Character:FindFirstChild("Accessory (defaultAccessory)") then
	game.Players.LocalPlayer.Character["Accessory (defaultAccessory)"].Handle.AccessoryWeld:Destroy()
	game.Players.LocalPlayer.Character["Accessory (Slug Rifle Black)"].Handle.AccessoryWeld:Destroy()
	AlignCharacter(game.Players.LocalPlayer.Character["Accessory (defaultAccessory)"].Handle, game.Players.LocalPlayer.Character.Scythe.Base, Vector3.new(0, 0, -3), Vector3.new(0, -90, 40))
	AlignCharacter(game.Players.LocalPlayer.Character["Accessory (Slug Rifle Black)"].Handle, game.Players.LocalPlayer.Character.Shotgun.Base, Vector3.new(0, -1, -0.5), Vector3.new(-20, 180, 0))
	for _, v in ipairs(game.Players.LocalPlayer.Character.Scythe:GetChildren()) do
		if v:IsA("BasePart") then
			v.Transparency = 1
		end
	end
	for _, v in ipairs(game.Players.LocalPlayer.Character.Shotgun:GetChildren()) do
		if v:IsA("BasePart") then
			v.Transparency = 1
		end
	end
else
	sendNotification("Moon Convert", "It is recommended to wear the hats used for this script... You can get the hats by using \"get hats;blind reaper\"", 7)
end
